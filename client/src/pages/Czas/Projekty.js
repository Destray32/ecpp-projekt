...
export default function ProjektyPage() {
    ...
    const handleDelete = (id) => {
        Axios.delete(`https://localhost:5000/api/czas/usun?id=${id}`, { withCredentials: true })
            .then((response) => {
                fetchProjects();
            })
            .catch((error) => {
    ...
    const handlePrzeniesAktyw = () => {
        Axios.put("https://localhost:5000/api/czas/przeniesAkt",
    ...
    const handlePrzeniesNieaktyw = () => {
        Axios.put("https://localhost:5000/api/czas/przeniesNieakt",
            { ids: selectedItems.map((id) => id) },
            { withCredentials: true }
        )
            .then((response) => {
