...
export default function PojazdyPage() {
    ...
    const confirmDelete = () => {
        axios.delete(`https://localhost:5000/api/pojazdy/${deleteItemId}`, { withCredentials: true })
            .then(() => {
    ...
    const handleEdit = (id) => {
        if (editableRow === id) {
            ...
            if (isValid) {
                setEditableRow(null);
                axios.put(`https://localhost:5000/api/pojazdy/${id}`, editedData, { withCredentials: true })
                    .then(() => {
