...
export default function HomePage() {
    ...
    const handleLogout = async () => {
        try {
            await axios.post('https://localhost:5000/api/logout', {}, { withCredentials: true });
            navigate('/');
        } catch (error) {
            console.error(error);
        }
    ...
    const daneUzupelnione = async () => {
        try {
            const response = await axios.get('https://localhost:5000/api/dane-uzupelnione', { withCredentials: true });
    ...
    const getBadgeCount = async () => {
        try {
            const response = await axios.get('https://localhost:5000/api/ogloszenia/count', { withCredentials: true });
    ...
    useEffect(() => { // useEffect ze względu na to żeby tata się zmieniała w razie potrzeby
        ...
        //     navigator.sendBeacon('https://localhost:5000/api/zamkniecieStrony');
        // };
