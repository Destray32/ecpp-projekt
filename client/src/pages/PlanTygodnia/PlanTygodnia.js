...
export default function PlanTygodniaPage() {
    ...
    const handlePrintSelectedGroups = () => {
        const url = `https://api-service-ecpp.onrender.com/api/planTygodnia/zaplanuj?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
    ...
    const handleUsunZaznaczone = async () => {
        try {
            ...
            await Axios.delete(`https://api-service-ecpp.onrender.com/api/planTygodnia`, {
    ...
    const handlePrzeniesZaznaczone = () => {
        ...
        Axios.put(`https://api-service-ecpp.onrender.com/api/planTygodnia`, payload, { withCredentials: true })
            .then(res => {
                fetchData();
                setSelectedRowIds([]);
            })
    ...
    const handleSkopiuj = async () => {
        try {
            ...
            const [currentWeekResponse, previousWeekResponse] = await Promise.all([
                Axios.get(`https://api-service-ecpp.onrender.com/api/planTygodnia/zaplanuj?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`, { withCredentials: true }),
                Axios.get(`https://api-service-ecpp.onrender.com/api/planTygodnia/zaplanuj?from=${encodeURIComponent(previousWeekStart)}&to=${encodeURIComponent(previousWeekEnd)}`, { withCredentials: true })
            ]);
            ...
            if (entriesToAdd.length > 0) {
                await Axios.post(`https://api-service-ecpp.onrender.com/api/planTygodniaPrev`, entriesToAdd, { withCredentials: true }); 
                fetchData();
    ...
    const fetchData = (selectedGroup = group) => {
        ...
        const url = `https://api-service-ecpp.onrender.com/api/planTygodnia/zaplanuj?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}${selectedGroup ? `&group=${encodeURIComponent(selectedGroup.name)}` : ''}`;
    ...
    const handleRadioChange = (employeeId, selectedM) => {
        ...
        Axios.put(`https://api-service-ecpp.onrender.com/api/planTygodnia/${employeeId}`, {
            M1_5: selectedM,
        }, { withCredentials: true })
            .then((res) => {
                fetchData();
