...
export default function ZaplanujTydzienPage() {
    ...
    const fetchPlany = useCallback(() => {
        ...
        Axios.get(`https://localhost:5000/api/planTygodnia/zaplanuj?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,
    ...
    const handleZaplanuj = () => {
        ...
        Axios.post('https://localhost:5000/api/planTygodnia/zaplanuj', {
    ...
    const handleUsun = (planId) => {
        Axios.delete(`https://localhost:5000/api/planTygodnia/zaplanuj?id=${planId}`, { withCredentials: true })
            .then((response) => {
                fetchPlany();
            })
            .catch((error) => {
