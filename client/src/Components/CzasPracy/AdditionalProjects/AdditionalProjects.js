...
const AdditionalProjects = ({
    ...
    const addWeek = async () => {
        ...
        try {
            ...
            const response = await Axios.post(`https://api-service-ecpp.onrender.com/api/czas/projekt`, {
        ...
        additionalProjects.forEach(project => {
            if (project.id === projectId) {
                ...
                Object.values(project.hours).forEach(async hour => {
                    try {
                        await Axios.delete(`https://api-service-ecpp.onrender.com/api/czas/projekt/${hour.id}`, { withCredentials: true });
                    } catch (error) {
