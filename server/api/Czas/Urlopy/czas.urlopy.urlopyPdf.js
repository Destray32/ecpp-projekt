function urlopyPdf(req, res, db) {
    ...
    if (!Array.isArray(Zleceniodawca) || Zleceniodawca.length === 0) {
        return res.status(400).json({ message: 'Brak zleceniodawców w zapytaniu' });
    }

    const placeholders = Zleceniodawca.map(() => '?').join(', ');
    const sql = `
        SELECT
            u.idUrlopy, 
            do.Imie,
    ...
    db.query(sql, Zleceniodawca, (err, result) => {
        if (err) {
            ...
            const data = result.map(row => ({
                ...
                Status: row.Status,
                FK_idPracownik: row.FK_idPracownik
            }));
