function GetProjekty(req, res, db) {
    ...
    const sql = 'SELECT * FROM projekty p JOIN Grupa_urlopowa gu ON p.Grupa_urlopowa_idGrupa_urlopowa = gu.idGrupa_urlopowa WHERE p.Archiwum = 0 AND p.Status = "Aktywny" ORDER BY p.data_dodania DESC';
    ...
    db.query(sql, (err, result) => {
        if (err) {
            ...
            const formattedRows = result.map(row => ({
                ...
                data_dodania_raw: row.data_dodania,
            }));
